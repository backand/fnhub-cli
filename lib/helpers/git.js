    ...
    isGit(destFolder, function (exists) {
        if (!exists) {
            ...
            status(destFolder, {}, function (err, result) {
                if (err) {
                    callback({message:util.format(fnhub.resources.Errors.Git.DamagedGitRepo,destFolder), expected:true});
                }
                else{
                    if (result.dirty.length > 0 && result.dirty[0].indexOf('module.yaml') == -1){ 
                        callback({message:util.format(fnhub.resources.Errors.Git.GitRepoHasUncommittedChanges, destFolder), expected:true});
                    }
                    else{
                        callback();
                    }
