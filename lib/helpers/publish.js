...
function validateYaml(moduleJson, callback){
    console.log('validateYaml');
    callback(null, moduleJson);
}

function validateGit(moduleJson, callback){
    console.log('validateGit');
    callback(null, moduleJson);
}
...
function startPublish(moduleJson, callback){
    ...
    var publishArgs = {name:name, version:version, githubRepo:githubRepo, status:'start'};
    console.log(publishArgs);

    backand.fn.post("publish", publishArgs)
