...
var BelongToSomeoneElseError = 'Module name belong to someone else';
...
function startPublish(moduleJson, fnhub, callback){
    ...
    backand.fn.post(publishLambda.name, {name:name, version:version, githubRepo:githubRepo, keywords:keywords, language:language, path:publishLambda.start})
    ...
    .catch(function(err){
        ...
        else if (err.data.errorMessage == BelongToSomeoneElseError) {
            callback({message: fnhub.resources.Errors.Publish.BelongToSomeoneElseError, expected:true}, null);
        }
        else if (err.data == fnhub.Consts.AuthenticationIsRequired){
            callback({message:fnhub.resources.Errors.Publish.AuthenticationIsRequired, expected:true}, null);
        }
...
function endPublish(version, temporaryRole, codeUri, fnhub, callback){
    ...
    deleteZip(fnhub.config.codeZipFileName);
    backand.fn.post(publishLambda.name, {versionId:version.__metadata.id, path:publishLambda.end})
    .then(function(response){
