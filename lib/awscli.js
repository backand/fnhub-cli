    ...
    exec(command, {cwd: process.cwd(), maxBuffer: opts.maxBuffer}, function (err, stdout, stderr) {
        ...
        if (stderr) {
            if (stderr.indexOf('DELETE_IN_PROGRESS') > 0) {
                return callback({message: fnhub.resources.Errors.Cf.Delete.InProgress, expected: true});
            }
            else {
                return callback(stderr);
            }
        }
        if (err) return callback({awscliDeployError:{err:err, stdout:stdout, stderr:stderr}})
        var response = { lines: [] }
            ...
            if (line.indexOf('Failed') == 0) {
                describeStackEvents(fnhub, stackName, function(err, response){
                    if (!response.StackEvents || !response.StackEvents.length) {
                        callback({message: util.format(fnhub.resources.Errors.Cf.Deploy.MissingStack, stackName), expected: true});
                    }
                ...
                describeStack(fnhub, stackName, function(err, response){
                    ...
                    if (!endpoints || !endpoints.length){
                        callback({message: util.format(fnhub.resources.Errors.Cf.Deploy.MissingOrIncomprehensiveOutputsOfStack, stackName), expected: true});
                    }
...
var describeStackEvents = function(fnhub, stackName, callback) {
    ...
    exec(command, {cwd: process.cwd(), maxBuffer: opts.maxBuffer}, function (err, stdout, stderr) {
        if (err) return callback({awscliDescribeEventsError:{err:err, stdout:stdout, stderr:stderr}})
        if (stderr) return callback(stderr)
...
var describeStack = function(fnhub, stackName, callback) {
    ...
    exec(command, {cwd: process.cwd(), maxBuffer: opts.maxBuffer}, function (err, stdout, stderr) {
        if (err) return callback({awscliDescribeError:{err:err, stdout:stdout, stderr:stderr}})
        if (stderr) return callback(stderr)
