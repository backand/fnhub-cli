...
function collectOptions(options, callback){
	...
	options.name = options.name || readlineSync.question(Questions.Init.Name, {
		defaultInput: path.basename(process.cwd()),
	}) || path.basename(process.cwd());

	if (options.defaultAuthor) {
		options.author = options.author || readlineSync.question(Questions.Init.Author, {
			defaultInput: options.defaultAuthor
		}) || options.defaultAuthor;
	}
	else {
		options.author = options.author || readlineSync.question(Questions.Init.AuthorNo);
	}
	delete options.defaultAuthor;

	options.version = options.version ||  readlineSync.question(Questions.Init.Version,{
		defaultInput: '1.0.0'
	}) || '1.0.0';
		
	options.description = options.description || readlineSync.question(Questions.Init.Description);
	...
	options.repo = options.repo || readlineSync.question(Questions.Init.GitRepo, {
		defaultInput: options.defaultRepo
	}) || options.defaultRepo;
	delete options.defaultRepo;

	options.keywords = options.keywords || readlineSync.question(Questions.Init.Keywords, {
		defaultInput: options.name
	}) || options.name;

	options.keywords = options.keywords ? _.words(_.replace(options.keywords, /[;|,]/g, ' '), /[^ ]+/g) : [" "];
	
	options.license = options.license ||  readlineSync.question(Questions.Init.License, {
		defaultInput: 'ISC'
	}) || 'ISC';
