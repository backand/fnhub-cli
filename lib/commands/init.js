...
function getCurrentUsername(options, fnhub, callback){
...
function collectOptions(options, fnhub, callback){
	...
	options.name = options.name || readlineSync.question(fnhub.resources.Questions.Init.Name, {
		defaultInput: path.basename(process.cwd()),
	}) || path.basename(process.cwd());

	if (options.defaultAuthor) {
		options.author = options.author || readlineSync.question(fnhub.resources.Questions.Init.Author, {
			defaultInput: options.defaultAuthor
		}) || options.defaultAuthor;
	}
	else {
		options.author = options.author || readlineSync.question(fnhub.resources.Questions.Init.AuthorNo);
	}
