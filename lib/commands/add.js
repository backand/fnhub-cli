...
function collectOptions(options, callback){
	...
	options.name = options.name || readlineSync.question(Questions.Add.Name, {
	...
	options.description = options.description || readlineSync.question(Questions.Add.Description, {
	...
	if(!options.description || options.description.length === 0){
		...
		options.description = readlineSync.question(Questions.Add.Description);
	...
	if (!options.runtime){
		...
		index = readlineSync.keyInSelect(runtime, Questions.Add.Runtime, {cancel: false});
	...
	if(!options.handler) {
		...
		options.handler = readlineSync.question(Questions.Add.Handler, {
			defaultInput: defaultHandler[index]
		});
	}

	if(!options.env){ //only ask if the object was not provided
		options.env = readlineSync.question(Questions.Add.Env);
		try{
			if(options.env) {
