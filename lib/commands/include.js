...
var includeHelper  		= require('../helpers/include.js');
...
module.exports = function(options){
    ...
    includeHelper.include(options, function(err, response){
        if (err) {
            ...
            logger.success(Messages.Include[options.provider].AfterSuccess.replace('{{0}}', response.templateName));
            process.exit(0);
        }
    });
}
